...
good_identifier = db.Table('good_identifier',
    db.Column('good_id', db.Integer, db.ForeignKey('goods.good_id')),
...
class User(db.Model):
    ...
    goods = db.relationship("Good",
                               secondary=good_identifier)
    isOwner = db.Column(db.Boolean,default=False, nullable=False);

    city = db.Column(db.String(100))

    def __init__(self, first_name, last_name, birth_date,isOwner,city):
...
def User_HomePage(id):
    ...
    goods = Good.query.filter(Good.city == "Paris")
    return render_template("user.html", employees=all_data, Goods=goods)
...
def insert_user():
        if request.method == 'POST':
            ...
            isOwner=  bool(request.form['isOwner'])


            city = request.form['city']

            my_data = User(first_name,last_name, birth_date,isOwner,city)
...
@app.route('/insert_good/<id>', methods=['POST'])
def insert_good(id):
        if request.method == 'POST':
